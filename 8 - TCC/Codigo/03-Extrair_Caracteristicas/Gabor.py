...
arquivo_csv = '../data/caracteristicas/GABOR_[PATCHES-500-COLUNAS]_[02-CLASSES].csv' # MUDAR AQUI
diretorio_caracteristicas = '../data/caracteristicas/'
arquivo_metadata = '../data/metadata.csv'

# Quantidade de classes
QUANT_CLASSES = 2 # MUDAR AQUI
...
def main():
    ...
    if key in classes:
      ...
      for image_name in os.listdir(key_path):
        ...
        if latitude is not None and longitude is not None:
          ...
          data_to_append = pd.Series([key, nome_arquivo_original, latitude, longitude] + list(caracteristicas_gabor))
  ...
  colunas = ['key', 'nome_arquivo', 'latitude', 'longitude'] + [f'GABOR_{i}' for i in range(len(caracteristicas_gabor))]
  df.columns = colunas

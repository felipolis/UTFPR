...
from tensorflow.keras.applications import MobileNet  # Importe a MobileNet em vez da ResNet50
from tensorflow.keras.models import Model
from tensorflow.keras.layers import Flatten
from tensorflow.keras.preprocessing import image
from skimage import feature
from tensorflow.keras.applications.mobilenet import preprocess_input  # Importe a função de pré-processamento da MobileNet
from sklearn.neighbors import KernelDensity
from scipy.spatial import distance

# Diretórios
diretorio_imagens = '../data/patches/5_patches' # MUDAR AQUI
arquivo_csv = '../data/caracteristicas/MOBILENET_[05-PATCHES]_[10-CLASSES].csv' # MUDAR AQUI
...
def extrair_caracteristicas_MobileNet(imagem):
    # Redimensione a imagem para o tamanho esperado pelo modelo MobileNet
    imagem = cv2.resize(imagem, (224, 224))
    # Pré-processamento da imagem de acordo com as especificações da MobileNet
    imagem = preprocess_input(imagem)
    # Expanda as dimensões da imagem para corresponder ao input_shape do modelo
    imagem = np.expand_dims(imagem, axis=0)
    # Obtenha as características usando o modelo MobileNet
    features = mobilenet_model.predict(imagem)
  ...
  for key in os.listdir(diretorio_imagens):
    if key in classes:
      ...
      for image_name in os.listdir(key_path):
        ...
        latitude, longitude = extrair_lat_long(arquivo_metadata, filename)  
        ...
        if latitude is not None and longitude is not None:
          ...
          caracteristicas_MobileNet = extrair_caracteristicas_MobileNet(imagem)

          # Adicione as características ao DataFrame
          data_to_append = pd.Series([key, nome_arquivo_original, latitude, longitude] + list(caracteristicas_MobileNet))
      ...
      colunas = ['key', 'nome_arquivo', 'latitude', 'longitude'] + [f'MOBILENET_{i}' for i in range(len(caracteristicas_MobileNet))]
      df.columns = colunas
...
if __name__ == '__main__':
    # Carregue o modelo MobileNet com os parâmetros desejados

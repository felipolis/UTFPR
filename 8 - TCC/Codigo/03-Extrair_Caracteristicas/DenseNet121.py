...
from tensorflow.keras.applications import DenseNet121  # Importe a DenseNet em vez da ResNet50
from tensorflow.keras.models import Model
from tensorflow.keras.layers import Flatten
from tensorflow.keras.preprocessing import image
from skimage import feature
from tensorflow.keras.applications.densenet import preprocess_input  # Importe a função de pré-processamento da DenseNet
from sklearn.neighbors import KernelDensity
from scipy.spatial import distance

# Diretórios
diretorio_imagens = '../data/patches/5_patches' # MUDAR AQUI
arquivo_csv = '../data/caracteristicas/DENSENET121_[05-PATCHES]_[10-CLASSES].csv' # MUDAR AQUI
...
def extrair_caracteristicas_DenseNet121(imagem):
    # Redimensione a imagem para o tamanho esperado pelo modelo DenseNet121
    imagem = cv2.resize(imagem, (224, 224))
    # Pré-processamento da imagem de acordo com as especificações da DenseNet
    imagem = preprocess_input(imagem)
    # Expanda as dimensões da imagem para corresponder ao input_shape do modelo
    imagem = np.expand_dims(imagem, axis=0)
    # Obtenha as características usando o modelo DenseNet121
    features = densenet_model.predict(imagem)
...
def main():
    ...
    if key in classes:
      ...
      for image_name in os.listdir(key_path):
        ...
        if latitude is not None and longitude is not None:
          ...
          caracteristicas_DenseNet = extrair_caracteristicas_DenseNet121(imagem)

          # Adicione as características ao DataFrame
          data_to_append = pd.Series([key, nome_arquivo_original, latitude, longitude] + list(caracteristicas_DenseNet))
  ...
  colunas = ['key', 'nome_arquivo', 'latitude', 'longitude'] + [f'DENSENET121_{i}' for i in range(len(caracteristicas_DenseNet))]
  df.columns = colunas
